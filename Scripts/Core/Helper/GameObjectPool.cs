...
namespace GJC.Helper
{
    ...
    public class GameObjectPool : SingleTon<GameObjectPool>
    {
        ...
        private Dictionary<GameObject, List<Coroutine>> coroutineDir;

        public override void init()
        {
            base.init();
            pool = new Dictionary<string, List<GameObject>>();
            coroutineDir = new Dictionary<GameObject, List<Coroutine>>();
        }
        ...
        {
            var cor = StartCoroutine(ReleaseIE(go, delay));
            if (!coroutineDir.ContainsKey(go))
                coroutineDir.Add(go, new List<Coroutine>());
            coroutineDir[go].Add(cor);
        }

        public void CancelRelease(GameObject go)
        {
            if (!coroutineDir.ContainsKey(go)) return;
            var cors = coroutineDir[go];
            for (int i = 0; i < cors.Count; i++)
            {
                StopCoroutine(cors[i]);
            }

            coroutineDir.Remove(go);
        }
